    ...
    def on_mouse_click(self, event, x, y, flags, param):
        if event == cv2.EVENT_LBUTTONDOWN:
            for i, pos in enumerate(self.button_positions):
                top_left = pos
                bottom_right = (
                    top_left[0] + self.button_width, top_left[1] + self.button_height)
...
class ImageSender(Node):
    def __init__(self):
        ...
        self.publisher6 = self.create_publisher(String, 'send_topic', 1)
    ...
    def send_image_to_topic(self, topic_number):
        if self.last_received_image is not None:
            if topic_number == 1:
                ...
                self.publisher6.publish(String())
...
def main(args=None):
    ...
    while rclpy.ok():
        ...
        if cv_image is not None:
            ...
            try:
                selection_image = bridge.cv2_to_imgmsg(
            ...
            if image_receiver.point != None:
                button_handler.clicked_image(
                    image_receiver.point.x, image_receiver.point.y)
